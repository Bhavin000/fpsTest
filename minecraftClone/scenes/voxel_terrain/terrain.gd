...
onready var voxel = [
	preload("res://scenes/voxel_terrain/grass.tscn"),
	preload("res://scenes/voxel_terrain/ground.tscn"),
	preload("res://scenes/voxel_terrain/water.tscn"),
	preload("res://scenes/voxel_terrain/stone.tscn")
]
...
func generate_water(pos):
	var v = voxel[2].instance()
	add_child(v)
	v.global_transform.origin = Vector3(pos.x, 2, pos.z)
	arr[Vector2(pos.x, pos.z)].append(2)
			...
			if arr.has(temp_posXZ):
				for k in arr[temp_posXZ].keys():
					if k > generation_depth + pos.y and k < generation_height + pos.y:
						idx_ = arr[temp_posXZ][k][0]
						key_ = arr[temp_posXZ][k][1]
						if arr_instance[idx_] is int and arr_instance[idx_] == 0:
							if deleted_arr.has(temp_posXZ) and k in deleted_arr[temp_posXZ]:
								pass
							else:
								arr_instance[idx_] = voxel[key_].instance()
								add_child(arr_instance[idx_])
								temp_pos = Vector3(pos.x, 0, pos.z) + Vector3(i,k,j)
								arr_instance[idx_].global_transform.origin = temp_pos
			else:
				...
				self.add_child(arr_instance[index])
				arr_instance[index].global_transform.origin = temp_pos
				arr[temp_posXZ] = {y:[index, key_]}
		...
		for k in range(4):
			...
			if arr.has(temp_posXZ):
				for j in arr[temp_posXZ].keys():
					idx_ = arr[temp_posXZ][j][0]
					if not arr_instance[idx_] is int:
						arr_instance[idx_].queue_free()
						arr_instance[idx_] = 0
...
func add_chunk(chnk, normal, key_):
	...
	if arr.has(temp_posXZ):
		if temp_pos.y in arr[temp_posXZ].keys():
			var idx_ = arr[temp_posXZ][temp_pos.y][0]
			if arr_instance[idx_] is int:
				arr_instance[idx_] = voxel[key_].instance()
				add_child(arr_instance[idx_])
				arr_instance[idx_].global_transform.origin = temp_pos
				arr[Vector2(temp_pos.x, temp_pos.z)][temp_pos.y][1] = key_
		else:
			arr_instance.append(voxel[key_].instance())
			add_child(arr_instance[index])
			arr_instance[index].global_transform.origin = temp_pos
			arr[Vector2(temp_pos.x, temp_pos.z)][temp_pos.y] = [index, key_]
			index += 1
